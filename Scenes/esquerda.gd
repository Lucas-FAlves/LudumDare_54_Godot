...
@export var move_direction = Vector2.RIGHT  # Direção inicial
var is_moving = false  # Indica se o RigidBody2D está se movendo
@export var move_limit = Vector2 (100,0)  # Limite para onde o RigidBody2D deve se mover
...
func _process(delta):
	if is_moving:
		...
		if global_position.x >= initialPosition.x:
			is_moving = false
			print("parou")
